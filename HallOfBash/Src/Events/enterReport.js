				...
				request(message.content, async function (error, response, body) {
					...
					if (!(attackerName === undefined || bashpoints === undefined) && skrMember.includes(attackerName)) {
						if (sFakeAmount <= 1000 && attackerName != defenderName) {
							...
							await conn(
								'INSERT INTO `hallofbash` (world, discordName, discordUserId, igAccountName, bashPoints, luck, moral, attackerXCoords, attackerYCoords, defenderXCoords, defenderYCoords, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
								[
									world,
