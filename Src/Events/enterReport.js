...
module.exports = {
	...
	run: async (message, client) => {
		...
		if (
			!message.author.bot &&
			message.content.startsWith(`https://${tableName}`) &&
			...
			let timeRegex =
				/K.*\s+(?<day>\d+).(?<month>\d+).(?<year>\d+)\s+(?<hour>\d+):(?<minute>\d+):(?<second>\d+)/g;
			let luckData = /Angreiferglück\s+(?<glueck>[^%]*)%/g;
			let moralData = /Moral\:\s+(?<moral>\d+)/g;
			let attData =
				/Angreifer:\s+(?<attackerName>.*?)(\s+)Herkunft\:\s+(?<attackerVillageName>[^(]*)\s\((?<attackerXCoord>\d+)\D(?<attackerYCoord>\d+)\)\sK(?<attackerContinent>\d+)/g;
			...
			let defenderRegex =
				/Verteidiger\:\s+(?<defenderName>.*)\s+Ziel\:\s+(?<defenderVillageName>[^(]*)\((?<defenderXCoord>\d+)\D(?<defenderYCoord>\d+)\)\sK(?<defenderContinent>\d+)\s+/g;
			...
			let kataRegex =
				/Schaden\s+durch\s+Katapultbeschuss:\D+(?<buildingOld>\d+)\D+(?<buildingNew>\d+)/g;
			console.log(link);
			...
			if (link.match(linkRegex)) {
				...
				request(message.content, async function (error, response, body) {
					...
					while ((matchDefender = defenderRegex.exec(input)) !== null) {
						defenderName = matchDefender.groups.defenderName
							? matchDefender.groups.defenderName
							: "Verteidigername";
						defenderVillageName = matchDefender.groups.defenderVillageName
							? matchDefender.groups.defenderVillageName
							: "Dorfname";
						defenderXCoords = matchDefender.groups.defenderXCoord
							? matchDefender.groups.defenderXCoord
							: 0;
						defenderYCoords = matchDefender.groups.defenderYCoord
							? matchDefender.groups.defenderYCoord
							: 0;
						defenderContinent = matchDefender.groups.defenderContinent
							? matchDefender.groups.defenderContinent
							: 0;
					}

					let matchAngreifer, sFakeAmount;
					while ((matchAngreifer = attackerUnitRegex.exec(input)) !== null) {
						let angreiferSpeerAmount = matchAngreifer.groups.anzahlSpeerAngreifer
							? matchAngreifer.groups.anzahlSpeerAngreifer
							: 0;
						let angreiferSchwertAmount = matchAngreifer.groups.anzahlSchwertAngreifer
							? matchAngreifer.groups.anzahlSchwertAngreifer
							: 0;
						let angreiferAxtAmount = matchAngreifer.groups.anzahlAxtAngreifer
							? matchAngreifer.groups.anzahlAxtAngreifer
							: 0;
						let angreiferSpyAmount = matchAngreifer.groups.anzahlSpyAngreifer
							? matchAngreifer.groups.anzahlSpyAngreifer
							: 0;
						let angreiferLeichteAmount = matchAngreifer.groups.anzahlLeichteAngreifer
							? matchAngreifer.groups.anzahlLeichteAngreifer
							: 0;
						let angreiferSchwereAmount = matchAngreifer.groups.anzahlSchwereAngreifer
							? matchAngreifer.groups.anzahlSchwereAngreifer
							: 0;
						let angreiferRammeAmount = matchAngreifer.groups.anzahlRammeAngreifer
							? matchAngreifer.groups.anzahlRammeAngreifer
							: 0;
						let angreiferKatapultAmount = matchAngreifer.groups.anzahlKatapultAngreifer
							? matchAngreifer.groups.anzahlKatapultAngreifer
							: 0;
						let angreiferPalaAmount = matchAngreifer.groups.anzahlPalaAngreifer
							? matchAngreifer.groups.anzahlPalaAngreifer
							: 0;
						let angreiferAGAmount = matchAngreifer.groups.anzahlAGsAngreifer
							? matchAngreifer.groups.anzahlAGsAngreifer
							: 0;
						...
						if (message.guild.id === config.server2.serverId) {
							...
							let angreiferBogenAmount = matchAngreifer.groups.anzahlBogenAngreifer
								? matchAngreifer.groups.anzahlBogenAngreifer
								: 0;
							let angreiferBeritteneAmount = matchAngreifer.groups
								.anzahlBeritteneAngreifer
								? matchAngreifer.groups.anzahlBeritteneAngreifer
								: 0;
							sFakeAmount =
								parseInt(angreiferSpeerAmount) +
								parseInt(angreiferSchwertAmount) +
					...
					while ((matchVerteidiger = defenderUnitRegex.exec(input)) !== null) {
						let verlustSpeerVerteidiger = matchVerteidiger.groups
							.verlusteSpeerVerteidiger
							? matchVerteidiger.groups.verlusteSpeerVerteidiger
							: 0;
						let verlustSchwertVerteidiger = matchVerteidiger.groups
							.verlusteSchwertVerteidiger
							? matchVerteidiger.groups.verlusteSchwertVerteidiger
							: 0;
						let verlustAxtVerteidiger = matchVerteidiger.groups.verlusteAxtVerteidiger
							? matchVerteidiger.groups.verlusteAxtVerteidiger
							: 0;
						let verlustSpyVerteidiger = matchVerteidiger.groups.verlusteSpyVerteidiger
							? matchVerteidiger.groups.verlusteSpyVerteidiger
							: 0;
						let verlustLeichteVerteidiger = matchVerteidiger.groups
							.verlusteLeichteVerteidiger
							? matchVerteidiger.groups.verlusteLeichteVerteidiger
							: 0;
						let verlustSchwereVerteidiger = matchVerteidiger.groups
							.verlusteSchwereVerteidiger
							? matchVerteidiger.groups.verlusteSchwereVerteidiger
							: 0;
						let verlustRammbockVerteidiger = matchVerteidiger.groups
							.verlusteRammbockVerteidiger
							? matchVerteidiger.groups.verlusteRammbockVerteidiger
							: 0;
						let verlustKatapultVerteidiger = matchVerteidiger.groups
							.verlusteKatapultVerteidiger
							? matchVerteidiger.groups.verlusteKatapultVerteidiger
							: 0;
						let verlustPalaVerteidiger = matchVerteidiger.groups.verlustePalaVerteidiger
							? matchVerteidiger.groups.verlustePalaVerteidiger
							: 0;
						let verlustAGVerteidiger = matchVerteidiger.groups.verlusteAGVerteidiger
							? matchVerteidiger.groups.verlusteAGVerteidiger
							: 0;
						let verlustMilizVerteidiger = matchVerteidiger.groups
							.verlusteMilizVerteidiger
							? matchVerteidiger.groups.verlusteMilizVerteidiger
							: 0;
						...
						if (message.guild.id === config.server2.serverId) {
							...
							let verlustBogenVerteidiger = matchVerteidiger.groups
								.verlusteBogenVerteidiger
								? matchVerteidiger.groups.verlusteBogenVerteidiger
								: 0;
							let verlustBeritteneVerteidiger = matchVerteidiger.groups
								.verlusteBeritteneVerteidiger
								? matchVerteidiger.groups.verlusteBeritteneVerteidiger
								: 0;
							bashpoints =
								parseInt(verlustSpeerVerteidiger * 4) +
								parseInt(verlustSchwertVerteidiger * 5) +
					...
					if (
						!(attackerName === undefined || bashpoints === undefined) &&
						skrMember.includes(attackerName)
					) {
						if (message.guild.id === config.server2.serverId) {
							if (attackerName != defenderName) {
								await channel.send(
									`${message.author}, ${inlineCode(
										attackerName
									)} hat mit diesem Angriff **${bashpoints} Bashis** gemacht.`
								);
								await conn(
								...
								await channel.send(
									`${message.author}, ${inlineCode(
										attackerName
									)} hat mit diesem Angriff **${bashpoints} Bashis** gemacht, dieser Bericht wird nicht für die Hall of Bash gewertet. (${sFakeAmount})`
								);
							}
						} else {
							if (sFakeAmount <= 1000 && attackerName != defenderName) {
								await channel.send(
									`${message.author}, ${inlineCode(
										attackerName
									)} hat mit diesem Angriff **${bashpoints} Bashis** gemacht. (${sFakeAmount})`
								);
								await conn(
									`INSERT INTO \`${tableName}\` (world, discordName, discordUserId, igAccountName, bashPoints, luck, moral, attackerXCoords, attackerYCoords, defenderXCoords, defenderYCoords, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
									[
						...
						if (message.guild.id === config.server2.serverId) {
							if (attackerName != defenderName) {
								await channel.send(
									`${message.author}, ${inlineCode(
										attackerName
									)} hat mit diesem Angriff **${palaXPBash} Bashis** gemacht.`
								);
								await conn(
									`INSERT INTO \`${tableName}\` (world, discordName, discordUserId, igAccountName, bashPoints, luck, moral, attackerXCoords, attackerYCoords, defenderXCoords, defenderYCoords, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
									[
								...
								await channel.send(
									`${message.author}, ${inlineCode(
										attackerName
									)} hat mit diesem Angriff **${palaXPBash} Bashis** gemacht, dieser Bericht wird nicht für die Hall of Bash gewertet. (${sFakeAmount})`
								);
							}
						} else {
							if (sFakeAmount <= 1000 && attackerName != defenderName) {
								await channel.send(
									`${message.author}, ${inlineCode(
										attackerName
									)} hat mit diesem Angriff **${palaXPBash} Bashis** gemacht. (${sFakeAmount})`
								);
								await conn(
									`INSERT INTO \`${tableName}\` (world, discordName, discordUserId, igAccountName, bashPoints, luck, moral, attackerXCoords, attackerYCoords, defenderXCoords, defenderYCoords, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
									[
						...
						await channel.send(
							`${message.author}, dieser Bericht wird nicht für die Hall of Bash gewertet.`
						);
					}
				});
			}
